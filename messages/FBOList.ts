...
function FBORow(f: OnAirFbo, i: number, t: Table) {
    ...
    const fuelAvailable = (f.AllowFuel100LLSelling) ? `${AllowFuel100LLSelling} ${f.Fuel100LLQuantity.toFixed(2)}/${f.Fuel100LLCapacity} ($${f.Fuel100LLSellPrice.toFixed(2)}/per gal)` : '';
    const fuelJetAvailable = (f.AllowFuelJetSelling) ? `${AllowFuelJetSelling} ${f.FuelJetQuantity.toFixed(2)}/${f.FuelJetCapacity} ($${f.FuelJetSellPrice.toFixed(2)}/per gal)` : '';
...
export function FBOList (x:OnAirFbo[]):string|void {
    ...
    x.forEach((f: OnAirFbo, i: number) => FBORow(f, i, t));    
    response = t.toString();
